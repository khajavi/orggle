...
### Fixed
- Fixed symlink creation to use absolute paths instead of relative paths
- Fixed bash shebang to use `#!/usr/bin/env bash` for NixOS compatibility
- Fish wrapper now correctly follows symlinks to resolve real installation directory
- Installer scripts now work correctly on systems where `/bin/bash` doesn't exist
